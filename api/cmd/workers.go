...
func (q *queueScheduler) drain(ctx context.Context) {
	...
	for _, candidate := range queued {
		...
		if !queuedSourceEntitled(ctx, q.users, user, plan, candidate) {
			candidate.Status = jobs.StatusFailed
			candidate.Error = "your current plan no longer supports this download source"
			if q.repo.Update(ctx, candidate) == nil {
				q.bus.Publish(events.JobFailed, events.Failed{JobID: candidate.ID, Reason: candidate.Error})
			}
			continue
		}
